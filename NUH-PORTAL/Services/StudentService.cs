...
namespace NUH_PORTAL.Services
{
    ...
    public class StudentService : AppServiceBase, IStudentService
    {
        ...
        // ⚠️ الشاشة دي كانت **مافيهاش أي فحص للسكن** أصلًا: لا سعة ولا تطابق
        //    بين الدور والشقة والغرفة. القائمة المنسدلة كانت الحارس الوحيد،
        //    وأي نداء مباشر للـ API بيعدّيها. والتسكين دلوقتي من طريق واحد
        //    لكل النظام - IHousingPlacement.
        ...
        public async Task<StudentDto> CreateAsync(StudentCreateDto dto)
        {
            ...
            // ⚠️ التحقّق من البنية والسعة مع بعض في IHousingPlacement - نفس
            //    اللي بيتنفّذ عند اعتماد الإسكان وعند النقل. الحدّ الأقصى مسموح:
            //    ده مسار الموظف لا الطالب.
            var housingError = await _placement.ValidateAsync(dto.housing_building, dto.floor_number,
                                    dto.apartment_number, dto.room_number,
                                    excludeStudentId: null, allowExceptionSlot: true);
            if (housingError != null) errors.Add(housingError);
        ...
        public async Task<StudentDto> UpdateAsync(int id, StudentUpdateDto dto)
        {
            ...
            //    مرتين في غرفته.
            // ⚠️ التسكين من الطريق الواحد: بيتحقّق ويكتب ويربط المفتاح الأجنبي.
            //    والقيم بتتاخد من **السجل بعد التعديل** لا من الـ dto: التعديل
            //    ممكن يغيّر خانة واحدة بس (رقم الغرفة مثلًا) والباقي من السجل،
            //    ففحص الـ dto وحده كان هيقارن غرفة جديدة بمبنى فاضي.
            await _placement.ApplyAsync(student, student.housing_building, student.floor_number,
                                        student.apartment_number, student.room_number,
                                        allowExceptionSlot: true,
                                        source: HousingHistoryKinds.Sources.StudentsScreen);
