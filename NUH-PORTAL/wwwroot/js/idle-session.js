...
var NuhIdle = (function () {
  ...
  // ⚠️ إنهاء الجلسة على **الخادم** لا على المتصفح وحده.
  //
  //    اللي كان بيحصل: لما العدّاد يوصل صفر، الكود كان بيعمل تحويل بس على
  //    /Account/Login والكوكي (NUH.Auth، ٣٠ دقيقة منزلقة) لسه صالح تمامًا.
  //    وصفحة الدخول أول سطر فيها بتشوف إن فيه كوكي شغّال فبترجّع المستخدم
  //    على /Home مسجّل دخول. النتيجة عند المستخدم: «العدّاد خلص وما حصلش حاجة».
  //
  //    الحلّ: نبعت POST حقيقي على /Account/Logout قبل التحويل. بنستعمل fetch
  //    لا submit للنموذج — إرسال النموذج بينقّل المتصفح بنفسه ولو الكوكي كان
  //    انتهى بيروح على Login?ReturnUrl=%2FAccount%2FLogout وده اللوب القديم.
  //    الـ fetch مابيحرّكش الصفحة، فإحنا اللي بنقرّر نروح فين بعده مهما كانت
  //    النتيجة (نجح، أو الكوكي كان راح خلاص — الحالتين نهايتهم واحدة).
  function serverSignOut() {
    try {
      var body = '';
      var tok = document.querySelector('form[action="/Account/Logout"] input[name="__RequestVerificationToken"]')
             || document.querySelector('input[name="__RequestVerificationToken"]');
      if (tok) body = '__RequestVerificationToken=' + encodeURIComponent(tok.value);
  ...
  function logout(expired) {
    ...
    // الخروج على الخادم أولًا في الحالتين — الخمول والضغط اليدوي — وبعده نقرّر
    // شكل الشاشة. من غير السطر ده الكوكي بيفضل حيّ وأي تحويل بيرجع للداخل.
    var done = serverSignOut();
    ...
    // ⚠️ replace لا href: المستخدم لو ضغط «رجوع» مايرجعش على الصفحة المحمية
    //    اللي كان فيها. و expired=1 عشان صفحة الدخول تقوله ليه اتقفلت جلسته
    //    بدل ما يلاقي نفسه على شاشة دخول بلا سبب.
    //    ⚠️ ومع expired يُرسَل مسار الشاشة الحالية ليعود إليها بعد الدخول:
    //       الخروج للخمول يقع والموظف في منتصف عمله، وإعادته إلى الصفحة
    //       الرئيسية تجعله يبحث عن موضعه من جديد. والخادم يفحص الوجهة
    //       (SafeReturnUrl) فلا تُقبل وجهة خارجية ولا مسار خروج أو دخول.
    var here = window.location.pathname + window.location.search;
    var to = expired
      ? '/Account/Login?expired=1&returnUrl=' + encodeURIComponent(here)
      : '/Account/Login';
    done.then(function () { window.location.replace(to); },
              function () { window.location.replace(to); });
  }
