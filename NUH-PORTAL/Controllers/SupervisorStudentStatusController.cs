...
namespace NUH_PORTAL.Controllers
{
    ...
    public class SupervisorStudentStatusController : ControllerBase
    {
        ...
        public SupervisorStudentStatusController(AppDbContext context, ActiveDirectoryService adService, IWebHostEnvironment env)
        {
            _context = context;
            ...
            IFormFile? file)
        {
            ...
            if (string.IsNullOrEmpty(st) || !validStatuses.Contains(st))
                return BadRequest(new { message = "نوع الحالة غير صحيح" });
            ...
            _context.AccountLifecycleLogs.Add(new AccountLifecycleLog
            {
                ...
                Details = $"Supervisor departure - {statusAr}: {notes.Trim()}" +
        ...
        public async Task<IActionResult> GetRecent()
        {
            ...
            var attachments = await _context.StudentStatusAttachments
                .Where(at => actionIds.Contains(at.StudentStatusActionId))
                .ToListAsync();
            foreach (var at in attachments)
            {
                if (!attachmentMap.ContainsKey(at.StudentStatusActionId))
            ...
            return Ok(actions.Select(a => new
            {
                ...
                AttachmentFileName = attachmentMap.ContainsKey(a.Id) ? attachmentMap[a.Id] : null
            }));
        }
    }
}
